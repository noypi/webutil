...
func TestRendererCloneTemplates(t *testing.T) {
	...
	webutil.MergePagesData(data, base, custom)
	tpl := oRenderer.CloneTemplate(base, custom)
	...
	// test 2
	data = map[string]interface{}{}
	webutil.MergePagesData(data, layout)
	tpl = oRenderer.CloneTemplate(layout)
	buf = new(bytes.Buffer)
	...
	// test 3
	data = map[string]interface{}{}
	webutil.MergePagesData(data, layout, base, custom)
	tpl = oRenderer.CloneTemplate(layout, base, custom)
	buf = new(bytes.Buffer)
